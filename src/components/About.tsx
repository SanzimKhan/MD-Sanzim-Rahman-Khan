          ...
          <div className="space-y-6 animate-fade-in">
            ...
            <div className="space-y-3">
              <div className="flex items-start gap-3">
                ...
                <p className="text-muted-foreground">Experienced in ROS, Python, C++, and embedded systems</p>
              </div>
              <div className="flex items-start gap-3">
                <div className="w-2 h-2 bg-primary rounded-full mt-2" />
                <p className="text-muted-foreground">Specialized in autonomous navigation and robotic manipulation</p>
              </div>
              <div className="flex items-start gap-3">
                <div className="w-2 h-2 bg-primary rounded-full mt-2" />
                <p className="text-muted-foreground">Passionate about STEM education and mentorship</p>
              </div>
            </div>
          </div>
          ...
          <div className="grid sm:grid-cols-2 gap-6">
            {stats.map((stat, index) => {
              ...
              return (
                <Card
                  key={index}
                  className="p-6 text-center hover:shadow-lg hover:card-glow transition-all duration-300 animate-fade-in bg-card/50 backdrop-blur-sm"
                  style={{ animationDelay: `${index * 100}ms` }}
                >
                  <div className="w-14 h-14 mx-auto mb-4 rounded-full bg-primary/10 flex items-center justify-center">
                    <Icon className="h-7 w-7 text-primary" />
                  </div>
                  <h4 className="text-3xl font-bold mb-2 gradient-text">{stat.value}</h4>
